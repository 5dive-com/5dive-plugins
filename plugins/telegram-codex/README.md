...
- v0.2.4 — `PLUGIN_VERSION` now reads from `package.json` at startup instead of a hardcoded const, so `/ping` / `/status` / `setMyCommands` report the actual shipped version. Previous PATCH bumps shipped the code but `/status` kept showing `0.2.1`. (this)
