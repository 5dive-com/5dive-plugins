...
- v0.2.2 — quieter user-facing pings: silence-watchdog now says `⏳ still working…` instead of dumping `N tool calls in, Xs since last reply` (telemetry read like debug output). `notification-relay` drops transient `RetryState` payloads and, when the upstream hands us a Rust `Debug`-formatted `SessionNotification { ... }` blob, extracts the inner `reason:` field instead of forwarding the whole struct. (this)
