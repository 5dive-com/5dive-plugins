...
function armTurnEndKick(): void {
  ...
  pendingTurnEndKick = setInterval(() => {
    ...
    process.stderr.write('telegram-codex: turn ended with inbound still queued, waking listen loop\n')
    kickListenLoop()
  }, TURN_END_POLL_MS)
...
function ingestInboxFile(name: string): void {
  ...
  try { obj = JSON.parse(raw) } catch {
    process.stderr.write(`telegram-codex: bad agent-inbox file ${name}: not JSON\n`); return
  }
  const text = typeof obj?.text === 'string' ? obj.text : ''
  if (!text.trim()) {
    process.stderr.write(`telegram-codex: agent-inbox file ${name} has no text\n`); return
  }
...
function startAgentInbox(): void {
  ...
  try {
    ...
    process.stderr.write(`telegram-codex: watching agent-inbox at ${AGENT_INBOX_DIR}\n`)
  } catch (err) {
    process.stderr.write(`telegram-codex: agent-inbox watch failed: ${err}\n`)
  }
}
...
async function handleSlashCommand(ctx: Context, text: string): Promise<boolean> {
  ...
  try {
    switch (cmd) {
      ...
      case 'start':
        await md(
          'This bot bridges Telegram to your OpenAI Codex session.\n\n' +
          'Already paired? Just type. Messages here reach the Codex session.\n\n' +
          ...
          'without 5dive: `bun pair.ts` in the telegram-codex plugin dir.\n\n' +
...
bot.on('callback_query:data', async ctx => {
  ...
  // inbound — the exact path a typed reply takes (enqueueInbound). Re-gate the
  // tapper (a leaked button could otherwise let a stranger answer); drop the
  // keyboard after so it can't be double-tapped, message text intact.
  ...
  // Tap-to-answer for a human-gate ping (DIVE-117 parity, DIVE-118). A tap on a
  // decision(--options)/approval gate's inline button lands as
  // `tna:<taskId>:<token>` — token is the option INDEX (resolved against the LIVE
  // need_options, never the tapped payload: dodges the 64-byte callback_data cap
  // and can't be tampered) or 'approved'/'denied'. The DB is source of truth:
  // re-read the gate so a dashboard/CLI answer or a double-tap can't
  // double-answer. Claimed here before the tgcodex permission-button match below;
  // fully fail-soft. (Emit is type-gated CLI-side — lands dormant until
  // task_need_notify enables buttons for codex; DIVE-118.)
        ...
        properties: {
          timeout_seconds: {
            ...
            description:
              ...
              + "Keep the default instead of asking for longer.",
          },
        },
      },
    },
...
function kickListenLoop(): void {
  ...
  void (async () => {
    ...
    process.stderr.write('telegram-codex: rearm submit not observed, retrying once\n')
    if (await kickListenLoopOnce(name)) { rearmSubmitFailed = false; return }
    rearmSubmitFailed = true
    process.stderr.write('telegram-codex: rearm submit still not observed — agent is not accepting input\n')
  })()
}
...
// it and never exits — one leaked bun process per fresh heartbeat nudge, a slow
// process/mem leak on any codex agent that picks up heartbeat tasks. Exiting on
// EOF frees the slot so the current MCP-connected spawn acquires it (single
// healthy poller = the live session's own bridge). shutdown() only unlinks the
// PID/heartbeat files it actually owns, so a parked spawn exiting here can never
// stomp the incumbent's slot.
...
if (SEND_ONLY) {
  ...
  process.stderr.write(
    `telegram-codex: SEND_ONLY — getUpdates disabled (team-bot member; the shared listener is the sole poller)\n`,
  )
} else void (async () => {
      ...
      if (!incumbentHolds()) {
        try {
          ...
          if (prev > 1 && prev !== process.pid && pidAlive(prev)) {
            process.stderr.write(`telegram-codex: reclaiming stale poller pid=${prev}\n`)
