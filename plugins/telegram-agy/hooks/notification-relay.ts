#!/usr/bin/env bun
/**
 * Antigravity `Notification` hook — relays error-flavored notifications to
 * Telegram so the user finds out when Antigravity crashes, hits a rate limit,
 * or otherwise fails mid-turn.
 *
 * Wired by the plugin's hooks/hooks.json (Notification event). Antigravity pipes
 ...
 * `AGY_NOTIFY_RELAY_ALL=1` to relay every notification.
 *
 * Disable entirely with `AGY_NOTIFY_RELAY_DISABLED=1`. Always exits
...
if (process.env.AGY_NOTIFY_RELAY_DISABLED === '1') exitContinue()

let payload: any = {}
try { payload = JSON.parse(readFileSync(0, 'utf8')) } catch {}

// Pull a meaningful text out of whatever shape Antigravity hands us. Antigravity's
...
// Antigravity sometimes hands us a `Debug`-formatted Rust struct (e.g. the full
...
// RetryState is agy's own retry machinery announcing an in-flight retry.
...
const relayAll = process.env.AGY_NOTIFY_RELAY_ALL === '1'
...
const body = `⚠️ agy: ${text.slice(0, 3500)}`
