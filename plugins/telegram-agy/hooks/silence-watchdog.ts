#!/usr/bin/env bun
/**
 * Antigravity `PreToolUse` hook — pings the user if Antigravity has been working
 ...
 * Opt-in by setting `AGY_SILENCE_WATCHDOG_ENABLED=1`.
 *
 ...
 * AGY_SILENCE_WATCHDOG_MS. Each subsequent ping in the SAME stretch
 ...
 *
 * Knobs:
 *   - AGY_SILENCE_WATCHDOG_ENABLED=1   → opt-in (default off)
 *   - AGY_SILENCE_WATCHDOG_DISABLED=1  → bypass entirely (back-compat alias for the new default)
 *   - AGY_SILENCE_WATCHDOG_MS=N        → BASE threshold in ms (default 600000 = 10 min)
 *
...
if (process.env.AGY_SILENCE_WATCHDOG_ENABLED !== '1') exitContinue()
if (process.env.AGY_SILENCE_WATCHDOG_DISABLED === '1') exitContinue()
...
const BASE_MS = Math.max(10_000, Math.min(3_600_000,
  Number(process.env.AGY_SILENCE_WATCHDOG_MS ?? 600_000)))
