...
// against access.json allowFrom before sending. Antigravity is polling-only (no
...
function kickListenLoop(): void {
  ...
  void (async () => {
    ...
    process.stderr.write('telegram-agy: rearm submit not observed, retrying once\n')
    if (await kickListenLoopOnce(name)) { rearmSubmitFailed = false; return }
    rearmSubmitFailed = true
    process.stderr.write('telegram-agy: rearm submit still not observed — agent is not accepting input\n')
  })()
}
...
installLifecycle({ channel: 'telegram-agy', stateDir: STATE_DIR, cleanup: shutdown })

// DIVE-1251: exit when our MCP parent (the agy/TUI session) disconnects. On
...
const onParentDisconnect = () => {
  if (shuttingDownExit) return
  process.stderr.write('telegram-agy: MCP parent disconnected (stdin EOF) — exiting orphaned bridge\n')
  shutdown()
}
      ...
      if (!incumbentHolds()) {
        try {
          ...
          if (prev > 1 && prev !== process.pid && pidAlive(prev)) {
            process.stderr.write(`telegram-agy: reclaiming stale poller pid=${prev}\n`)
