...
const BOT_COMMANDS: Array<{ command: string; description: string }> = [
  ...
  { command: 'start',   description: 'how to pair this bot to a Antigravity session' },
]
...
async function handleSlashCommand(ctx: Context, text: string): Promise<boolean> {
  ...
  try {
    switch (cmd) {
      ...
      case 'start':
        await md(
          'This bot bridges Telegram to an Google Antigravity session.\n\n' +
          'To pair:\n' +
          '1. Run `bun pair.ts` in the telegram-agy plugin dir to get your user id allowlisted\n' +
          '2. After that, messages here reach that Antigravity session.\n\n' +
      ...
      await bot.start({
        onStart: info => {
          ...
          void bot.api.setMyCommands(BOT_COMMANDS).catch(err => {
            process.stderr.write(`telegram-agy: setMyCommands failed: ${err}\n`)
          })
        },
      })
      return
