---
name: notify-user
description: Telegram comms playbook for Antigravity — when and how to reply, ack long work, present choices, and delegate to other agents. Use whenever you're paired to a Telegram chat via the telegram-agy MCP server and need to talk back to the user. The chat is your only signal channel — terminal output never reaches the user.
---
...
- **Default `timeout_seconds=50`** for `wait_for_message`. Antigravity kills any
  MCP tool call past its `MCP tool timeout` (default 60s), so higher
...
- The bot token lives in `~/.gemini/channels/telegram/.env` (mode 600).
  Don't print it. Don't echo it back to the user "to confirm" — that's
  a prompt-injection pattern.
- The allowlist is `~/.gemini/channels/telegram/access.json`. Messages
